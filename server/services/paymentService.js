...
class PaymentService {
  ...
  async getMercadoPagoPaymentDetails(paymentId) {
    try {
      logger.info(
        "🔍 Consultando API real do MercadoPago para paymentId:",
        paymentId
      );

      // Consultar API real do MercadoPago
      const mercadopagoService = require("../config/mercadopago");
      const realPaymentDetails = await mercadopagoService.getPaymentStatus(
        paymentId
      );

      if (!realPaymentDetails) {
        logger.error(
          "❌ Erro ao consultar API do MercadoPago para paymentId:",
          paymentId
        );
        return null;
      }
      ...
      if (!results) {
        logger.error(
          "❌ Pedido não encontrado no banco para payment_id:",
          paymentId
        );
        return null;
      }
  ...
  async activateVIPForPayment(paymentDetails) {
    try {
      ...
      const plan =
        this.extractPlanFromDescription(paymentDetails.description) ||
        orderResult.plan_id;
      ...
      // Ativar VIP
      const vipResult = await this.activateVIP(
        user,
        planInfo,
        paymentDetails.id
      );
  ...
  async processPaymentWebhook(webhookData, headers = {}, rawBody = "") {
    try {
      ...
      if (eventAction === "payment.created") {
        logger.info(
          "📋 Evento payment.created recebido - PIX foi gerado, aguardando pagamento"
        );
      } else if (eventAction === "payment.updated") {
        logger.info(
          "📋 Evento payment.updated recebido - Status do pagamento foi alterado"
        );
      }
      ...
      switch (paymentStatus) {
        ...
        case "pending":
          logger.info(
            "⏳ Pagamento PENDENTE - apenas atualizando status:",
            eventId
          );
  ...
  async processPaymentEvent(paymentData, action) {
    try {
      ...
      if (vipResult.success) {
        ...
        await this.createPaymentConfirmationNotification(
          user_id,
          plan,
          payment_id
        );
  ...
  async processChargebackEvent(chargebackId, action) {
    try {
      logger.info("Processando evento de chargeback:", {
        chargebackId,
        action,
      });
  ...
  extractPlanFromDescription(description) {
    ...
    // Planos diários - Pré-Jogo
    if (desc.includes("pre-daily") || desc.includes("pré jogo diário"))
      return "pre-daily";
    if (desc.includes("pre-weekly") || desc.includes("pré jogo semanal"))
      return "pre-weekly";
    if (desc.includes("pre-monthly") || desc.includes("pré jogo mensal"))
      return "pre-monthly";
    if (desc.includes("pre-yearly") || desc.includes("pré jogo anual"))
      return "pre-yearly";

    // Planos diários - Live
    if (desc.includes("live-daily") || desc.includes("live diário"))
      return "live-daily";
    if (desc.includes("live-weekly") || desc.includes("live semanal"))
      return "live-weekly";
    if (desc.includes("live-monthly") || desc.includes("live mensal"))
      return "live-monthly";
    if (desc.includes("live-yearly") || desc.includes("live anual"))
      return "live-yearly";

    // Planos diários - Pré+Live
    if (desc.includes("prelive-daily") || desc.includes("pré + live diário"))
      return "prelive-daily";
    if (desc.includes("prelive-weekly") || desc.includes("pré + live semanal"))
      return "prelive-weekly";
    if (desc.includes("prelive-monthly") || desc.includes("pré + live mensal"))
      return "prelive-monthly";
    if (desc.includes("prelive-yearly") || desc.includes("pré + live anual"))
      return "prelive-yearly";

    // Planos diários - Valuebet
    if (desc.includes("value-daily") || desc.includes("valuebet diário"))
      return "value-daily";
    if (desc.includes("value-weekly") || desc.includes("valuebet semanal"))
      return "value-weekly";
    if (desc.includes("value-monthly") || desc.includes("valuebet mensal"))
      return "value-monthly";
    if (desc.includes("value-yearly") || desc.includes("valuebet anual"))
      return "value-yearly";

    // Planos diários - Full
    if (desc.includes("full-daily") || desc.includes("full diário"))
      return "full-daily";
    if (desc.includes("full-weekly") || desc.includes("full semanal"))
      return "full-weekly";
    if (desc.includes("full-monthly") || desc.includes("full mensal"))
      return "full-monthly";
    if (desc.includes("full-yearly") || desc.includes("full anual"))
      return "full-yearly";
    ...
    try {
      ...
      if (existingVIP) {
        ...
        const newExpiresAt = new Date(
          existingVIP.data_fim.getTime() + plan.days * 24 * 60 * 60 * 1000
        );
      ...
      const {
        invalidateUserCache,
      } = require("../utils/optimizedPlanMiddleware");
      invalidateUserCache(user.id.toString());

      logger.info("📦 Cache invalidado após ativação VIP:", {
        userId: user.id,
      });
  ...
  async isWebhookAlreadyProcessed(webhookId) {
    try {
      ...
      if (!tableExists[0].exists) {
        logger.warn(
          "⚠️ Tabela webhook_events não existe, pulando verificação de duplicação"
        );
        return false;
      }
  ...
  async markWebhookAsProcessed(webhookId, paymentId, status) {
    try {
      ...
      if (!tableExists[0].exists) {
        logger.warn(
          "⚠️ Tabela webhook_events não existe, pulando marcação de processamento"
        );
        return;
      }
  ...
  notifyPaymentConfirmedViaWebSocket(paymentDetails) {
    try {
      ...
      logger.info(
        "🔔 WebSocket notification sent for payment confirmation:",
        webSocketData
      );
    } catch (error) {
  ...
  async processPaymentWebhook(webhookData) {
    try {
      ...
      const { type, action, data, id } = webhookData;

      // Diferentes tipos de notificação do MercadoPago
      if (type === "payment") {
        return await this.processPaymentNotification(data.id);
      } else if (type === "plan" || type === "subscription") {
        return await this.processSubscriptionNotification(data.id);
      } else {
  ...
  async processPaymentNotification(paymentId) {
    try {
      logger.info("Processando notificação de pagamento ID:", paymentId);

      // Simulação de dados de pagamento (substituir por consulta real à API)
      const paymentData = {
        id: paymentId,
        status: "approved",
      ...
      if (paymentData.status === "approved") {
        ...
        if (global.notifyPaymentConfirmed) {
          global.notifyPaymentConfirmed({
            paymentId: paymentData.id,
            orderId: paymentData.external_reference,
            planName: paymentData.metadata.plan_id,
            status: "confirmed",
        ...
        if (global.notifyPaymentUpdate) {
          global.notifyPaymentUpdate({
            paymentId: paymentData.id,
            orderId: paymentData.external_reference,
            status: paymentData.status,
            amount: paymentData.transaction_amount,
        ...
        return {
          success: true,
          message: `Status do pagamento atualizado: ${paymentData.status}`,
          paymentId: paymentData.id,
          status: paymentData.status,
        };
      }

    ...
    <div class="login-card">
      ...
      <form v-if="isLoginMode" @submit.prevent="handleLogin" class="login-form">
        <div class="form-group">
          <input v-model="email" type="email" placeholder="E-mail" class="form-input" :class="{ 'error': emailError }"
            required @blur="validateEmail" />
          <span v-if="emailError" class="error-message">{{ emailError }}</span>
        </div>

        <div class="form-group">
          <input v-model="password" :type="showPassword ? 'text' : 'password'" placeholder="Senha" class="form-input"
            :class="{ 'error': passwordError }" required @blur="validatePassword" />
          <button type="button" class="password-toggle" @click="togglePassword">
            ...
            <svg v-else class="eye-icon" fill="currentColor" viewBox="0 0 16 16">
              <path
                d="m10.79 12.912-1.614-1.615a3.5 3.5 0 0 1-4.474-4.474l-2.06-2.06C.938 6.278 0 8 0 8s3 5.5 8 5.5a7.029 7.029 0 0 0 2.79-.588zM5.21 3.088A7.028 7.028 0 0 1 8 2.5c5 0 8 5.5 8 5.5s-.939 1.721-2.641 3.238l-.708-.709C13.327 10.665 14 9.665 14 8.5 14 6.567 12.433 5 10.5 5c-1.165 0-2.165.673-2.79 1.588l.5.5z" />
              <path
                d="M5.525 7.646a2.5 2.5 0 0 0 2.829 2.829l-2.83-2.829zm4.95.708-2.829-2.83a2.5 2.5 0 0 1 2.829 2.829zm3.171 3.172-2.828-2.829a3.5 3.5 0 0 0-4.95-4.95l-2.829-2.828a6.5 6.5 0 0 1 7.778 7.778z" />
            </svg>
          </button>
        ...
        <div class="button-group">
          <button type="submit" class="login-btn primary" :disabled="isLoading || !isFormValid">
            <span v-if="isLoading" class="loading-spinner"></span>
            <span v-else>{{ loginButtonText }}</span>
          </button>
          <button type="button" class="login-btn secondary" @click="toggleMode" :disabled="isLoading">
            Criar conta
          </button>
        </div>
      </form>
      ...
      <form v-else @submit.prevent="handleRegister" class="register-form">
        <div class="form-group">
          <input v-model="registerForm.name" type="text" placeholder="Nome completo" class="form-input"
            :class="{ 'error': registerForm.nameError }" required @blur="validateRegisterName" />
          <span v-if="registerForm.nameError" class="error-message">{{ registerForm.nameError }}</span>
        </div>

        <div class="form-group">
          <input v-model="registerForm.email" type="email" placeholder="E-mail" class="form-input"
            :class="{ 'error': registerForm.emailError }" required @blur="validateRegisterEmail" />
          <span v-if="registerForm.emailError" class="error-message">{{ registerForm.emailError }}</span>
        </div>

        <div class="form-group">
          <input v-model="registerForm.password" :type="showRegisterPassword ? 'text' : 'password'" placeholder="Senha"
            class="form-input" :class="{ 'error': registerForm.passwordError }" required
        ...
        <div class="form-group">
          <input v-model="registerForm.confirmPassword" :type="showConfirmPassword ? 'text' : 'password'"
            placeholder="Confirmar senha" class="form-input" :class="{ 'error': registerForm.confirmPasswordError }"
          ...
          <span v-if="registerForm.confirmPasswordError" class="error-message">{{ registerForm.confirmPasswordError
            }}</span>
        </div>
        ...
        <div class="button-group">
          <button type="submit" class="register-btn primary" :disabled="isLoading || !isRegisterFormValid">
            <span v-if="isLoading" class="loading-spinner"></span>
            <span v-else>Criar conta</span>
          </button>
          <button type="button" class="register-btn secondary" @click="toggleMode" :disabled="isLoading">
            Voltar ao login
          </button>
        </div>
      </form>

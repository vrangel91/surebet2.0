...
export function getBookmakerUrl(
  ...
  // 1. PRIORIDADE: Tenta encontrar URL específica para live se aplicável
  if (isLive && bookmakerLiveUrls[house]) {
    console.log(
      `🔴 URL Live encontrada para ${house}:`,
      bookmakerLiveUrls[house]
    );
    return bookmakerLiveUrls[house];
  }

  // 2. PRIORIDADE: Busca URL padrão no nosso mapeamento
  const url = bookmakerUrls[house];
  if (url) {
    console.log(`🏠 URL encontrada no mapeamento para ${house}:`, url);
    return url;
  }

  // 3. FALLBACK: Tenta variações específicas do nome no nosso mapeamento
  const variations = [
    house.toLowerCase(),
    house.toUpperCase(),
    house.replace(/\s+/g, ""),
  ...
  for (const variation of variations) {
    ...
    if (exactMatch) {
      console.log(
        `🔍 URL encontrada por busca exata ${variation}:`,
        exactMatch
      );
      return exactMatch;
    }
  ...
  if (anchorhUrl && typeof anchorhUrl === "string") {
    ...
    if (extractedUrl) {
      console.log(`🔗 URL extraída de anchorh para ${house}:`, extractedUrl);
      return extractedUrl;
    }
  }

  console.warn(
    `⚠️ URL não encontrada para casa: "${house}". Usando URL genérica.`
  );
 ...
 * @returns {string|null} URL extraída ou null se não for possível extrair
 */
export function extractUrlFromAnchorh(anchorh) {
  ...
  try {
    ...
    if (urlMatch && urlMatch[0]) {
      ...
      console.log(`🔗 URL extraída de anchorh:`, url);
      return url;
    }
    ...
    if (domainMatch && domainMatch[1]) {
      ...
      console.log(`🌐 URL construída a partir do domínio:`, url);
      return url;
    }

    console.warn(`⚠️ Não foi possível extrair URL de:`, anchorh);
    return null;
  } catch (error) {
    console.error(`❌ Erro ao extrair URL de anchorh:`, error);
    return null;
  }
}

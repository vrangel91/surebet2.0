...
export const bookmakerUrls = {
  ...
  "Jogão": "https://jogao.bet.br/sports",
  ...
  "Betboo": "https://www.betboo.bet.br/",
  "TivoBet": "https://tivo.bet.br/br/sportsbook/prematch",
  "Faz1Bet": "https://faz1.bet.br/br/sportsbook/prematch",
  ...
  "JonBet": "https://jonbet.bet.br/pt/sports",
  "BetVip": "https://betvip.bet.br/sports",
  "Rivalry": "https://www.rivalry.com/pt/esports",
  "7Games": "https://7games.bet.br/pb/sports/pre-match/event-view/Soccer/World/18277589",
  "Betão": "https://betao.bet.br/pb/sports/pre-match/event-view/Soccer/World/10579",
  ...
  "BetBra Exchange": "https://betbra.bet.br/b/exchange",
  "BolsaDeApostas": "https://bolsadeaposta.bet.br/b/exchange",
  "FulltBet": "https://fulltbet.bet.br/b/exchange",

  // URLs adicionais das casas principais
  "BFSportsbook": "https://betfast.bet.br/",
...
// URLs específicas para jogos live (diferentes das pre-match)
 ...
 */
export function getBookmakerUrl(house, isLive = false) {
  if (!house) {
    console.warn('Casa de apostas não informada')
    return '#'
  }

  // Tenta encontrar URL específica para live se aplicável
  if (isLive && bookmakerLiveUrls[house]) {
    console.log(`🔴 URL Live encontrada para ${house}:`, bookmakerLiveUrls[house])
  ...
  if (url) {
    console.log(`🏠 URL encontrada para ${house}:`, url)
    return url
  }

  // Se não encontrou, tenta variações do nome
  const variations = [
    house.toLowerCase(),
    house.toUpperCase(),
    house.replace(/\s+/g, ''),
    house.replace(/\..*$/, ''), // Remove extensões como .bet.br
    house.split('.')[0] // Pega apenas a primeira parte antes do ponto
  ]

  for (const variation of variations) {
    const foundUrl = Object.keys(bookmakerUrls).find(key => 
      key.toLowerCase().includes(variation.toLowerCase()) ||
      variation.toLowerCase().includes(key.toLowerCase())
    )
    
    if (foundUrl && bookmakerUrls[foundUrl]) {
      console.log(`🔍 URL encontrada por variação ${variation} -> ${foundUrl}:`, bookmakerUrls[foundUrl])
      return bookmakerUrls[foundUrl]
    }
  }

  console.warn(`⚠️ URL não encontrada para casa: "${house}". Usando URL genérica.`)
  return `https://www.google.com/search?q=${encodeURIComponent(house + ' apostas esportivas')}`
}
 ...
 * @param {boolean} isLive - Se é uma URL específica para live
 */
export function addBookmakerUrl(house, url, isLive = false) {
  if (isLive) {
    bookmakerLiveUrls[house] = url
    console.log(`➕ URL Live adicionada para ${house}:`, url)
  } else {
    bookmakerUrls[house] = url
    console.log(`➕ URL adicionada para ${house}:`, url)
  }
}
...
/**
 * Extrai o domínio de um campo anchorh da API
 * @param {string} anchorh - Valor do campo anchorh1 ou anchorh2
 * @returns {string|null} Domínio extraído ou null se não for possível extrair
 */
export function extractDomainFromAnchorh(anchorh) {
  ...
  try {
    // Busca pelo parâmetro domain= na URL
    const domainMatch = anchorh.match(/domain=([^&]+)/)
    if (domainMatch && domainMatch[1]) {
      const domain = domainMatch[1]
      console.log(`🔗 Domínio extraído de anchorh:`, domain)
      return domain
    }

    // Se não encontrar domain=, tenta extrair o domínio da URL completa
    const urlMatch = anchorh.match(/https?:\/\/([^\/\?&]+)/)
    if (urlMatch && urlMatch[1]) {
      const domain = urlMatch[1]
      console.log(`🌐 Domínio extraído da URL:`, domain)
      return domain
    }

    console.warn(`⚠️ Não foi possível extrair domínio de:`, anchorh)
    return null
  } catch (error) {
    console.error(`❌ Erro ao extrair domínio de anchorh:`, error)
    return null
  }
}

/**
 * Constrói a URL completa para uma casa de apostas baseada no domínio extraído
 * @param {string} domain - Domínio extraído do anchorh
 * @param {boolean} isLive - Se é um jogo live
...
export function buildBookmakerUrlFromDomain(domain, isLive = false) {
  ...
  // Se é live, tenta adicionar parâmetros específicos para jogos ao vivo
  if (isLive) {
    // Adiciona parâmetros comuns para jogos live
    const liveParams = ['/live', '/ao-vivo', '/in-play', '/live-betting']
    const hasLiveParam = liveParams.some(param => url.includes(param))
    
    if (!hasLiveParam) {
      // Adiciona o primeiro parâmetro live disponível
      url = url.endsWith('/') ? url + 'live' : url + '/live'
    }
  }

  console.log(`🏗️ URL construída para domínio ${domain}:`, url)
  return url
}

...
router.get('/', asyncHandler(async (req, res) => {
  ...
  // Determinar o tipo de usuário usando VIPService para verificação em tempo real
  let userType = 'basic';
  if (req.user.is_admin) {
    userType = 'admin';
  } else {
    // Verificar status VIP em tempo real usando VIPService
    const VIPService = require('../utils/vipService');
    try {
      const vipStatus = await VIPService.checkVIPStatus(req.user.id);
      if (vipStatus.hasVIP && !vipStatus.vipStatus?.isExpired) {
        userType = 'vip';
      }
    } catch (error) {
      console.warn('Erro ao verificar status VIP para notificações:', error);
      // Manter como basic em caso de erro
    }
  }
  ...
  // Consulta mais simples para evitar erros de SQL
  ...
  // Contar separadamente para evitar problemas com include
...
router.get('/unread-count', asyncHandler(async (req, res) => {
  ...
  if (req.user.is_admin) {
    ...
    try {
      ...
      console.warn('Erro ao verificar status VIP para contagem de notificações:', error);
      // Manter como basic em caso de erro
    }
  }
  ...
  res.json(successResponse(
    { unreadCount: count },
    'Contagem de notificações não lidas obtida com sucesso'
  ));
}));
